                    ...
                    .then((resData) => {
                        if (resData.exists && resData.patient) {
                            setDuplicateInfo({
                                ...
                                nomor_rekam_medis: resData.patient.nomor_rekam_medis,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (data.no_hp.trim() && !/^08\d{8,12}$/.test(data.no_hp.trim())) {
            addToast({
                ...
                description: 'Nomor HP tidak valid. Gunakan format: 08xx-xxxx-xxxx',
        ...
        post('/pasien', {
            onSuccess: () => {
                const randomRm = `RM-2026-${Math.floor(10000 + Math.random() * 90000).toString().substring(1)}`;
                addToast({
                    type: 'success',
            ...
            onError: () => {
                addToast({
                    type: 'error',
                    title: 'Pendaftaran Gagal',
                    description: 'Terjadi kesalahan sistem saat mendaftarkan pasien.',
                    actionText: 'Coba Lagi',
                });
            },
        });
            ...
            <div className="mx-auto max-w-4xl space-y-6">
                ...
                <div>
                    <div className="flex items-center gap-2 text-xs text-gray-500 font-medium mb-1">
                        <Link href="/dashboard" className="hover:text-[#145e5b] transition">
                            Dashboard
                        </Link>
                        <span>&rsaquo;</span>
                        <span className="text-gray-800 font-semibold">Pendaftaran Pasien</span>
                    </div>

                    <h1 className="text-2xl md:text-3xl font-bold font-serif text-gray-900 tracking-tight">
                        Pendaftaran Pasien Baru
                    </h1>
                    <p className="text-xs md:text-sm text-gray-500 mt-1">
                        Silakan lengkapi data pasien dengan benar untuk keperluan rekam medis.
                    </p>
                </div>
                    ...
                    <div className="rounded-xl border border-red-300 bg-red-50/90 p-4 shadow-2xs transition-all">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 text-xs">
                                <h3 className="font-bold text-red-800 text-xs md:text-sm">
                                    Perhatian: Potensi Data Ganda
                                </h3>
                                <p className="mt-1 text-red-700 leading-relaxed">
                                    NIK ini sudah terdaftar atas nama{' '}
                                    <span className="font-bold text-red-900">{duplicateInfo.nama_lengkap}</span>
                                    , Nomor RM:{' '}
                                    <span className="font-bold font-mono text-red-900">{duplicateInfo.nomor_rekam_medis}</span>.
                                </p>
                                <div className="mt-2">
                                    <Link
                ...
                {/* Main Card Container */}
                <div className="rounded-2xl border border-[#d3ece7] bg-white p-6 md:p-8 shadow-xs">
                    <form onSubmit={handleSubmit} className="space-y-8">
                        {/* Section 1: Data Pribadi */}
                        <div className="space-y-4">
                            <h2 className="border-b border-gray-200 pb-2 text-base font-bold font-serif text-gray-900">
                                Data Pribadi
                            </h2>
                            ...
                            <div>
                                ...
                                />
                                {errors.nama_lengkap && (
                                    <p className="mt-1 text-[11px] text-red-600">{errors.nama_lengkap}</p>
                                )}
                            </div>
                                ...
                                <div className="flex items-center gap-6">
                                    <label className="flex items-center gap-2 cursor-pointer text-xs font-medium text-gray-800">
                                        <input
                                            ...
                                            className="h-4 w-4 text-[#145e5b] focus:ring-[#145e5b] accent-[#145e5b]"
                                        />
                                        Laki-laki
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer text-xs font-medium text-gray-800">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        value={data.penjamin}
                                        onChange={(e) => setData('penjamin', e.target.value)}
                                        className="w-full rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-xs text-gray-900 focus:border-[#145e5b] focus:ring-1 focus:ring-[#145e5b] focus:outline-none"
                                    >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setData('no_hp_kontak_darurat', e.target.value)}
                                        placeholder="08xxx..."
                                        className="w-full rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-xs text-gray-900 focus:border-[#145e5b] focus:ring-1 focus:ring-[#145e5b] focus:outline-none"
                                    />
                                </div>
                        ...
                        <div className="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
                            <Link
                                href="/pasien"
                                className="px-5 py-2.5 text-xs font-semibold text-gray-700 hover:text-gray-900 transition"
                            >
                                Batal
                            </Link>
                            <button
                                type="button"
                                onClick={handleReset}
                                className="px-5 py-2.5 text-xs font-semibold text-[#145e5b] border border-[#145e5b] rounded-lg hover:bg-[#e4f6f2] transition"
                            >
                                Reset
                            </button>
                            <button
                                type="submit"
                                disabled={processing}
                                className="px-6 py-2.5 text-xs font-bold text-white bg-[#145e5b] hover:bg-[#0e4845] rounded-lg shadow-xs flex items-center gap-2 transition disabled:opacity-50"
                            >
