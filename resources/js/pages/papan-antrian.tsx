...
export default function PapanAntrian({ poliList = [] }: PapanAntrianProps) {
    ...
    const speakCall = (antrian: any) => {
        if (!antrian || typeof window === 'undefined' || !('speechSynthesis' in window)) return;
        
        try {
            window.speechSynthesis.cancel(); // Stop any pending speech
            const nomorSpaced = antrian.nomor_antrian ? antrian.nomor_antrian.split('').join(' ') : '';
            const namaPasien = antrian.pasien?.nama_lengkap || '';
            const namaPoli = antrian.poli?.nama_poli || 'Poliklinik';
            const namaDokter = antrian.dokter?.nama_lengkap ? `Dokter ${antrian.dokter.nama_lengkap}` : '';
        ...
        <div className="min-h-screen bg-slate-950 text-white font-sans flex flex-col justify-between p-6">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between border-b border-slate-800 pb-5 gap-4">
                <div className="flex items-center gap-4">
                    <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-teal-400 to-teal-600 text-slate-950 font-black flex items-center justify-center text-2xl shadow-lg shadow-teal-500/20">
                        RS
                    </div>
                    <div>
                        ...
                        <p className="text-xs text-teal-400 font-semibold tracking-wide flex items-center gap-2">
                            ...
                            <span>•</span>
                            <span className="text-emerald-400 font-medium">● LIVE SYNCHRONIZED DISPLAY</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        onClick={() => {
                            ...
                            if (nextState && sedangDipanggil) speakCall(sedangDipanggil);
                        }}
                    ...
                    >
                        ...
                        {isAudioEnabled ? 'Suara Pemanggilan: Aktif' : 'Suara Pemanggilan: Mute'}
                    </button>

                    <select
                        value={selectedPoliId}
                        onChange={(e) => setSelectedPoliId(e.target.value)}
                        className="rounded-xl bg-slate-900 border border-slate-700 px-4 py-2 text-xs font-semibold text-teal-300 outline-none focus:border-teal-500"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 my-auto py-4">
                ...
                <div className="lg:col-span-2 space-y-6 flex flex-col justify-between">
                    ...
                    <div className="rounded-3xl bg-gradient-to-br from-slate-900 via-teal-950/60 to-slate-900 border-2 border-teal-500/50 p-8 shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[300px]">
                        ...
                        <div className="flex items-center justify-between border-b border-teal-500/20 pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="rounded-full bg-teal-500/20 border border-teal-500/40 px-4 py-1 text-xs font-black text-teal-300 tracking-wider uppercase">
                            ...
                            {sedangDipanggil && (
                                <button
                                    ...
                                    className="px-3 py-1 bg-teal-500/20 hover:bg-teal-500/40 border border-teal-500/40 rounded-lg text-xs font-bold text-teal-300 transition-all flex items-center gap-1.5"
                                >
                        ...
                        {sedangDipanggil ? (
                            <>
                                <div className="my-6 text-center space-y-1">
                                    <span className="text-xs font-bold tracking-widest text-teal-400 uppercase">
                                        {sedangDipanggil.poli?.nama_poli || 'POLIKLINIK'}
                                    </span>
                                    <h2 className="text-8xl font-black text-transparent bg-clip-text bg-gradient-to-r from-teal-200 via-emerald-300 to-teal-400 tracking-tighter drop-shadow-2xl">
                                        {sedangDipanggil.nomor_antrian}
                                    </h2>
                                    <p className="text-lg font-bold text-slate-200 mt-2">
                                        Pasien: {sedangDipanggil.pasien?.nama_lengkap || '-'}
                                    </p>
                                </div>

                                <div className="grid grid-cols-2 gap-4 rounded-2xl bg-slate-900/90 border border-slate-800 p-4 backdrop-blur-md">
                                    <div>
                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                                            TUJUAN LOKET / RUANGAN
                                        </span>
                                        <span className="text-lg font-bold text-white block mt-0.5">
                                            {sedangDipanggil.loket?.nama_loket || sedangDipanggil.poli?.nama_poli || 'Ruang Dokter'}
                                        </span>
                                    </div>
                                    <div className="border-l border-slate-800 pl-4">
                                        ...
                                        <span className="text-lg font-bold text-teal-300 block mt-0.5">
                                            {sedangDipanggil.dokter?.nama_lengkap ? `dr. ${sedangDipanggil.dokter.nama_lengkap}` : 'Dokter Praktik'}
                                        </span>
                                    </div>
                                </div>
                            </>
                            ...
                            <div className="my-12 text-center space-y-3">
                                ...
                                <h3 className="text-xl font-bold text-slate-300">Belum Ada Pemanggilan Antrian</h3>
                                <p className="text-xs text-slate-400">Pemanggilan pasien dari loket atau ruang periksa akan ditampilkan otomatis di layar ini.</p>
                            </div>
                        )}
                    </div>
                            ...
                            {panggilanPerPoli.map((item, idx) => (
                                ...
                                >
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="text-xs font-bold text-teal-400 truncate">{item.poli?.nama_poli}</span>
                                        {item.antrian && (
                                    ...
                                    {item.antrian ? (
                                        <div>
                                            <p className="text-2xl font-black text-white">{item.antrian.nomor_antrian}</p>
                                            <p className="text-[11px] text-slate-300 font-semibold truncate">{item.antrian.pasien?.nama_lengkap}</p>
                                            <p className="text-[9px] text-slate-400 truncate mt-0.5">dr. {item.antrian.dokter?.nama_lengkap}</p>
                                        </div>
                                    ) : (
                                        <div className="py-2">
                                            <p className="text-xs text-slate-500 font-medium">Kosong / Menunggu</p>
                                        </div>
                                    )}
                                </div>
                            ))}
                ...
                <div className="rounded-3xl bg-slate-900/90 border border-slate-800 p-6 flex flex-col justify-between shadow-xl">
                    <div>
                        <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-4">
                            <div>
                                <h3 className="text-base font-bold text-white">DAFTAR ANTRIAN TUNGGU</h3>
                                <p className="text-[10px] text-slate-400">Urutan pasien berikutnya</p>
                            </div>
                            <span className="rounded-full bg-teal-500/20 border border-teal-500/30 px-3 py-1 text-xs font-bold text-teal-300">
                        ...
                        <div className="space-y-2.5 max-h-[460px] overflow-y-auto pr-1">
                            {daftarTunggu.length > 0 ? (
                                daftarTunggu.map((item, idx) => (
                                    <div
                                        key={idx}
                                        className="flex items-center justify-between rounded-2xl bg-slate-950 border border-slate-800/80 p-3 hover:border-slate-700 transition-colors"
                                    >
                                        <div className="flex items-center gap-3">
                                            <span className="h-9 w-9 rounded-xl bg-teal-500/10 border border-teal-500/30 text-teal-300 font-black flex items-center justify-center text-xs">
                                                {item.angka_antrian}
                                            </span>
                                            <div>
                                                ...
                                                <p className="text-[10px] text-slate-400">
                                                    {item.pasien?.nama_lengkap || 'Pasien'} • <span className="text-teal-400 font-semibold">{item.poli?.nama_poli}</span>
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <div className="py-16 text-center space-y-2 text-slate-500">
                                    ...
                                    <p className="text-xs font-semibold">Tidak Ada Pasien Menunggu</p>
                                    <p className="text-[10px]">Semua antrian telah selesai dilayani.</p>
                                </div>
                            )}
                        </div>
                    </div>

                    <div className="border-t border-slate-800 pt-3 text-center text-[10px] text-slate-400 font-medium">
            ...
            {/* Ticker Footer */}
            <div className="rounded-2xl bg-slate-900 border border-slate-800 p-3 overflow-hidden mt-4">
                <div className="whitespace-nowrap animate-marquee text-xs font-semibold text-slate-300 flex items-center gap-6">
                    ...
                    <span>Mohon perhatikan nomor antrian yang dipanggil di layar TV Display dan pengeras suara.</span>
                    <span>•</span>
                    <span>Pastikan berkas BPJS / Kartu Berobat sudah disiapkan saat memasuki ruang pemeriksaan.</span>
                    <span>•</span>
                    <span>Terima kasih atas kerja sama Anda di Rumah Sakit Sentosa Medika.</span>
                </div>
            </div>
        </div>
    );
